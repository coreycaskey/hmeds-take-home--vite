...
import { useGetProviderAppointmentsQuery } from '~/redux/api/api-slice';
import { formatIsoDateToTime, isFetchError } from '~/utils';

export interface ProviderAppointmentsProps {
  providerId: string;
  selectedDate: string;
}

export const ProviderAppointments: React.FC<ProviderAppointmentsProps> = ({
  providerId,
  selectedDate,
}) => {
        ...
        <List sx={{ width: '100%', bgcolor: 'background.paper' }}>
          {data.map((appointment) => (
            <ListItem key={appointment.id} alignItems="flex-start">
              ...
              <ListItemText
                primary={`Client: ${appointment.client.name}`}
                secondary={
                  <Typography
                    sx={{ display: 'inline' }}
                    component="span"
